...
class Main {
    public static void main(String[] args) {
        ...
        System.out.println(first(arr, 0, n - 1) > second(arr, 0, n - 1));
    }

    private static int first(int[] nums, int start, int end) {
        if (start == end) {
            return nums[start];
        } else if (start > end) {
            return 0;
    ...
    private static int second(int[] nums, int start, int end) {
        if (start >= end) {
            return 0;
        }

        // 第二手选的目的就是让first下一次选的时候的结果是最小的
        return Math.min(first(nums, start + 1, end), first(nums, start, end - 1));
    }
}